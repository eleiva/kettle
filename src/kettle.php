...
class ORM
{
    ...
    public function findMany(array $options = array())
    {
        ...
        if ($this->_filter_conditions) {
					...
					//FIX - Norton
            $options['KeyConditions'] = $filter_conditions;
        }
    ...
    public function count(array $options = array())
        ...
        if ($this->_filter_conditions) {
			...
			$filter = array(
          "TableName" 		=> "krakatoa_events",
          'Select'       	=> 'COUNT',
          'ScanFilter'   	=> $options['ScanFilter']
          );
			$result = self::$_client->scan($filter);
    ...
    public function query(array $conditions, array $options = array())
    {
        ...
        if (intval($this->_limit) > 0) { // Has limit
            ...
            //self::_logQuery("query", $args, null);
           ...
           // self::_logQuery("query", $args, $result);
            ...
            self::_logQuery('getIterator', $args, null );
    ...
    public function scan(array $options = array())
    {
			 $options['TableName'] = $this->_table_name;
				if ( $this->_limit ){
					$limit = array('limit' => $this->_limit);
        	$iterator             = self::$_client->getIterator('Scan', $options,$limit);
				} else {
        	$iterator             = self::$_client->getIterator('Scan', $options);
				}
        self::_logQuery("scan", $options, null);
        $items                = array();
        foreach ($iterator as $item) {
            $items[] = $item;
        }
    ...
    protected function _getDataType($key)
    {
		...
		public function setSchema($key,$dtype) {
				$this->_schema[$key] = $dtype;
		}
    /**
    ...
    protected static function _logQuery($query, array $args, $response)
    {
		...
		protected static function log($log){ 
		 // open file
		 $fd = fopen("/tmp/kettle.log", "a");
		 // write string
		 fwrite($fd, print_r($log,true) . "\n");
		 // close file
		 fclose($fd);
   }
}
